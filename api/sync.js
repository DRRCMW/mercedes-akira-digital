export default async function handler(req, res) {
  ...
  async function rGet(k) {
    if (!RU) return null;
    try { const r = await fetch(RU+'/get/'+encodeURIComponent(k),{headers:{Authorization:'Bearer '+RT}}); const d=await r.json(); return d.result?JSON.parse(d.result):null; } catch(e){return null;}
  }
  async function rSet(k,v) {
    if (!RU) return false;
    try { await fetch(RU+'/set/'+encodeURIComponent(k),{method:'POST',headers:{Authorization:'Bearer '+RT,'Content-Type':'application/json'},body:JSON.stringify(JSON.stringify(v))}); return true; } catch(e){return false;}
  }
  ...
  if (action==='notion-pull') {
    ...
    try {
      ...
      do {
        ...
        const nd=await nr.json();if(!nr.ok)return res.status(500).json({error:'Notion error',detail:nd});
  ...
  if (action==='status') {
    ...
    return res.status(200).json({ok:true,pipeline_count:pipeline.length,last_run:lastRun,redis_connected:!!RU,message:'Sync API working'});
  }
