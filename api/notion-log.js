...
// Called by: Make backup scenario, agent direct logging
// Falls back gracefully if token missing/expired â never breaks sales flow
...
export default async function handler(req, res) {
  ...
  const payload = JSON.stringify(body.payload || body.data || { source: fromAgent, ts: new Date().toISOString() });
  ...
  const properties = {
    ...
    'From Agent': { select: { name: validFrom.includes(fromAgent) ? fromAgent : 'System' } },
    Type: { select: { name: validType.includes(type) ? type : 'Update' } },
    Status: { select: { name: validStatus.includes(status) ? status : 'Done' } },
    Priority: { select: { name: validPriority.includes(priority) ? priority : 'Normal' } },
  };
  if (validTo.includes(toAgent)) properties['To Agent'] = { select: { name: toAgent } };
  ...
  try {
    ...
    return r.ok
      ...
      : res.status(200).json({ logged: false, notionError: data.message, code: data.code });
